...
public class BinarySearchTree {
    ...
    public void preOrder(Node node) { // top-down, left-right
        if(node == null) {
    ...
    public void inOrder() { // leaves-up, left branch then right branch
        if( this.isEmpty() )
            System.out.println(">>> The tree is empty <<<");
        else
            inOrder(root);
    ...
    public void postOrder(Node node) { // leaves, layer by layer. left then right 
        if(node == null) {
            //System.out.println("<null>");
            return;
        }
